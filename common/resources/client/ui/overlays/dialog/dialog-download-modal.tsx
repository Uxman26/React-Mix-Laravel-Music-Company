...
export type DialogDownloadModalSize =
  | InputSize
  | '2xl'
  | 'auto'
  | 'fullscreen'
...
export interface DialogDownloadModalProps
  extends Omit<ComponentPropsWithoutRef<'div'>, 'size'> {
  children: ReactNode;
  size?: DialogDownloadModalSize;
  background?: string;
  className?: string;
  radius?: string;
  maxWidth?: string;
}
export function DialogDownloadModal(props: DialogDownloadModalProps) {
  const {
    type = 'modal',
    dialogProps,
    ...contextProps
  ...
  return (
    ...
    >
      {Children.toArray(children).map(child => {
        if (isValidElement<DialogDownloadModalProps>(child)) {
          return cloneElement<DialogDownloadModalProps>(child, {
...
function sizeStyle(dialogSize?: DialogDownloadModalSize) {
  switch (dialogSize) {
    case '2xs':
      return 'w-256';
    case 'xs':
